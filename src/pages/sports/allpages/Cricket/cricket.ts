...
export class SportCricket implements OnDestroy {
  ...
  selectTab(index, event) {
    ...
    if(index == 0){
      jQuery('iframe').attr('src', 'http://indiaatsports.com/widget_matches?tournamentid=6294869374926848');
    }
    else if(index == 1){
      jQuery('iframe').attr('src', 'http://indiaatsports.com/widget_pointstable?tournamentid=6294869374926848');
    }
    else if(index == 2){
      jQuery('iframe').attr('src', 'http://indiaatsports.com/displayteamsandplayerslist?tournamentid=6294869374926848&orderingformat=byplayers');
    }
  }
