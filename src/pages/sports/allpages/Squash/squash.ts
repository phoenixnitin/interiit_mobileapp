...
export class SportSquash implements OnDestroy {
  // selectedItem: any;
  // squash: Array<object>;
  loading:any;
  // icons: string[];
  ...
  selectTab(index, event) {
    ...
    if(index == 0){
      jQuery('iframe').attr('src', 'http://indiaatsports.com/widget_matches?tournamentid=5624243618316288');
    }
    else if(index == 1){
      jQuery('iframe').attr('src', 'http://indiaatsports.com/widget_pointstable?tournamentid=5624243618316288');
    }
    else if(index == 2){
      jQuery('iframe').attr('src', 'http://indiaatsports.com/displayteamsandplayerslist?tournamentid=5624243618316288&orderingformat=byplayers');
    }
  }
